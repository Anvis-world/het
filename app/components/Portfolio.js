...
const techStacks = [
  ...
  { icon: "vite.svg", language: "Next.js" },
...
export default function Portfolio() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (certificateData.length > 0) setCertificates(certificateData);

      // Store in localStorage
      if (typeof window !== 'undefined') {
        localStorage.setItem("projects", JSON.stringify(projectData.length > 0 ? projectData : sampleProjects));
        localStorage.setItem("certificates", JSON.stringify(certificateData.length > 0 ? certificateData : sampleCertificates));
      }
    } catch (error) {
  ...
  return (
    <div className="md:px-[10%] px-[5%] w-full sm:mt-0 mt-[3rem] bg-[#030014] dark:bg-[#030014] light:bg-white overflow-hidden" id="Portfolio">
      {/* Header section */}
      <div className="text-center pb-10" data-aos="fade-up" data-aos-duration="1000">
        ...
        <p className="text-slate-400 dark:text-slate-400 light:text-slate-600 max-w-2xl mx-auto text-sm md:text-base mt-2">
